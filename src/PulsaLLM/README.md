...
| `Provider.Type` | `local`, `openai`, `openai-compatible` | `local` |
...
로컬에서 LLM을 실행합니다. GPU가 있으면 자동으로 활용합니다.

모델 프리셋: `default` (Phi-3.5 Mini), `fast` (Llama 3.2 1B), `quality` (Phi-4)
...
LM Studio, Ollama 등 OpenAI 호환 API를 사용합니다.
...
prompt 파일은 YAML frontmatter와 본문으로 구성됩니다. frontmatter는 선택 사항이며, `appsettings.json`의 provider 설정을 오버라이드합니다.
